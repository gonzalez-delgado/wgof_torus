...
#' pr <- geodesic_projection(u = c(-1, -2), data_1, data_2, do_plots = TRUE)
#' pr$plot_list[[1]]
#' pr$plot_list[[2]]
#' pr$plot_list[[3]]
#' pr$plot_list[[4]]
...
geodesic_projection<-function(u,data_1,data_2,do_plots=FALSE,size_points=1){
  ...
  if(do_plots){ # Produce plots illustrating the algorithm for the given samples and geodesic
    ...
    plot_list[[1]] <- ggplot2::ggplot(both_data, aes(x = phi, y = psi, col = sample))+
    ...
    plot_list[[2]] <- ggplot2::ggplot(both_data, aes(x = phi,y = psi,col = sample))+
      ggplot2::geom_point(size = size_points)+
      diag_list+
  ...
  if(do_plots){ # Produce plots illustrating the algorithm for the given samples and geodesic
    ...
    plot_list[[3]]  <- ggplot2::ggplot(both_data, aes(x = phi, y = psi, col = sample))+
      ggplot2::geom_point(size = size_points)+
      diag_list+
      ggplot2::scale_x_continuous(expand = c(-1,2))+
      ggplot2::scale_y_continuous(expand = c(-1,2))+
      ggplot2::theme(legend.position='none')+
      ggplot2::geom_point(aes(x = proj_phi, y = proj_psi, col = sample), size = 0.5)+
      ggplot2::ggtitle(paste0('Geodesic projection to (', u[1],',',ifelse(x_split, -u[2], u[2]),')'))+
      ggplot2::labs(x=expression(phi),y=expression(psi))
  ...
  if(do_plots){ # Produce plots illustrating the algorithm for the given samples and geodesic
    ...
    plot_list[[4]] <- ggplot2::ggplot(both_data, aes(x = phi, y = psi, col = sample))+
      ggplot2::geom_point(size = size_points)+
      diag_list+
      ggplot2::scale_x_continuous(expand = c(-1, 2))+
      ggplot2::scale_y_continuous(expand = c(-1, 2))+
    ...
    plot_list[[5]] <- ggplot2::ggplot(both_data, aes(x = phi, y = psi, col = sample))+
      ggplot2::geom_point(size = size_points)+
      diag_list+
      ggplot2::scale_x_continuous(expand = c(0, 0))+
      ggplot2::scale_y_continuous(expand = c(0, 0))+
      ggplot2::theme(legend.position = 'none')+
      ggplot2::geom_point(aes(x = proj_phi, y = proj_psi, col = sample), size = 0.8, alpha = 0.5)+
      ggplot2::ggtitle(paste0('Geodesic projection to (', u[1], ',', ifelse(x_split, -u[2], u[2]),')'))+
