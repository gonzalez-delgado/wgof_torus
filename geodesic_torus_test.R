...
#' 
#' sim_free_null <- sim.null.stat(500, NC = 4)
#' 
#' samp_1<-BAMBI::rvmcos(n) / (2 * pi) # Bivariate von Mises distributions
#' samp_2<-BAMBI::rvmcos(n) / (2 * pi)
#' 
#' #4 geodesics are chosen randomly
#' geodesic_torus_test(samp_1, samp_2, n_geodesics = 4, NC_geodesic = 4, sim_null = sim_free_null) 
#'
#' #4 geodesics are chosen a priori
#' glist <- list(c(1, 0), c(0, 1), c(1, 1), c(2, 3))
#' geodesic_torus_test(samp_1, samp_2, geodesic_list = glist, NC_geodesic = 4, sim_null = sim_free_null) 
#'
#' 
#' @export


geodesic_torus_test<-function(data_1, data_2, n_geodesics = 1, NC_geodesic = 1, geodesic_list = NULL, sim_null = NULL, NR = 500, NC = 1, n=30){
 
  one_geodesic_test <- function(u, data_1, data_2, sim_null_free){
    
    p <- geodesic_projection(u, data_1, data_2, do_plots = FALSE)
  ...
  parallel::clusterExport(cl=cl, varlist=c('one_geodesic_test', 'geodesic_projection', 'twosample.test.s1', 'sim_null',
  ...
  return(pbeta(min(all_projections), 1, n_geodesics))
